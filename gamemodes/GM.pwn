...
//KHUSUS PRAGMA
#pragma unused CreateDynamicObjectBank
#pragma unused AuctionStart
...
#include "MODULE\INGAMEMAP.pwn"
#include "MODULE\ROB.pwn"
#include "MODULE\ROBBANK.pwn"
...
#include "MODULE\LADANG.pwn"
#include "MODULE\SIREN.pwn"
//#include "MODULE\STORAGE.pwn"
...
#include "MODULE\JOB\JOB_SPAREPART.pwn"
...
public BotStatus()
{
	...
	format(statuz,sizeof(statuz),"%d/20 Players | Expensive Roleplay", pemainic, h, m);
...
{
 	new astring[48];
 	if(pData[playerid][pCs] == 0)format(astring, sizeof(astring), ""RED_E"Unverified");
...
public OnGameModeInit()
{
    ...
    SAVEPOS = DCC_FindChannelById("973240252162506822"); //
...
//	Ladang();
	...
	*/
    ...
    CreateDynamicPickup(1239, 23, 1186.41, -891.32, 43.30, -1, -1, -1, 50);
	format(strings, sizeof(strings), "[Dapur]\n{FFFFFF}/menumasak");
	CreateDynamic3DTextLabel(strings, COLOR_ORANGE2, 1186.41, -891.32, 43.30, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // pencucian uang haram

	CreateDynamicPickup(1239, 23, 1198.13, -889.93, 43.16, -1, -1, -1, 50);
	format(strings, sizeof(strings), "[List Menu]\n{FFFFFF}/menu -  untuk membuka list menu");
	CreateDynamic3DTextLabel(strings, COLOR_LBLUE, 1198.13, -889.93, 43.16, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // bank
...
/*	SAMDVehicles[3] = AddStaticVehicleEx(416, 1116.0294, -1296.6489, 13.6160, 179.4438, 1, 3, VEHICLE_RESPAWN, 1);
	...
	SAMDVehicles[10] = AddStaticVehicleEx(563, 1812.62, -1426.35, 36.18, 80.27, -1, 3, VEHICLE_RESPAWN, 1);
...
public OnGameModeExit()
{
	...
	print("-------------- [ Auto Gmx ] --------------");
	SendClientMessageToAll(COLOR_RED, "[!]"YELLOW_E" Sorry Server is Maintenance/Restart.{00FFFF} ~Expensive BOTS");
...
public OnPlayerCommandPerformed(playerid, cmd[], params[], result, flags)
{
    if (result == -1)
    {
        Error(playerid, "{d1d1d1}""%s"" doesn't exists! Use /help for more information", params);
        return 0;
    }
	new str[150];
...
public OnPlayerEnterDynamicCP(playerid, checkpointid)
{
	...
	{
		ShowPlayerDialog(playerid, DIALOG_BUYPVCP, DIALOG_STYLE_LIST, "{7fffd4}Expensive Showroom", "Motorcycle\nMobil\nKendaraan Sport\nKendaraan Job", "Select", "Cancel");
	}
			...
			if(pData[playerid][pSideJob] == 2)
			{
				...
				if(GetVehicleModel(vehicleid) == 431)
				{
					...
					else if(pData[playerid][pBus] == 35)
					{
						...
						pData[playerid][pCheckPoint] = CHECKPOINT_NONE;
						DisablePlayerRaceCheckpoint(playerid);
						AddPlayerSalary(playerid, "Sidejob(Bus)", 710);
						Info(playerid, "Sidejob(Bus) telah masuk ke pending salary anda!");
						RemovePlayerFromVehicle(playerid);
						SetTimerEx("RespawnPV", 3000, false, "d", vehicleid);
					}
			...
			if(pData[playerid][pSideJob] == 1)
			{
				...
				if(GetVehicleModel(vehicleid) == 574)
				{
					...
					else if(pData[playerid][pSweeper] == 20)
					{
						...
						pData[playerid][pCheckPoint] = CHECKPOINT_NONE;
						DisablePlayerRaceCheckpoint(playerid);
						AddPlayerSalary(playerid, "Sidejob(Sweeper)", 500);
						Info(playerid, "Sidejob(Sweeper) telah masuk ke pending salary anda!");
						RemovePlayerFromVehicle(playerid);
						SetTimerEx("RespawnPV", 3000, false, "d", vehicleid);
					}
			...
			if(pData[playerid][pSideJob] == 3)
			{
				...
				if(GetVehicleModel(vehicleid) == 530)
				{
					...
					else if(pData[playerid][pForklifter] == 11)
					{
						...
						DisablePlayerRaceCheckpoint(playerid);
						AddPlayerSalary(playerid, "Sidejob(Forklift)", 810);
			...
			if(pData[playerid][pSideJob] == 4)
			{
				...
				if(GetVehicleModel(vehicleid) == 572)
				{
					...
					else if(pData[playerid][pMower] == 12)
					{
						...
						pData[playerid][pMowerTime] += 150;
						pData[playerid][pCheckPoint] = CHECKPOINT_NONE;
						DisablePlayerRaceCheckpoint(playerid);
						AddPlayerSalary(playerid, "Sidejob(Mower)", 450);
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	else if(newstate == PLAYER_STATE_DRIVER)
    {
		...
		else if(pData[playerid][pHBEMode] == 2)
		{
			...
			PlayerTextDrawShow(playerid, KPH[playerid]);
	...
	if(IsValidVehicle(vehicleid))
	{
		...
		if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		{
			...
			else if(pData[playerid][pHBEMode] == 2)
			{
				...
				new Float:nDamage = fDamage / 10;
				if(GetEngineStatus(vehicleid))
				{
					PlayerTextDrawSetString(playerid, CRUISE[playerid], "~g~CRUISE");
				}
...
*/
				...
				PlayerTextDrawSetString(playerid, SPEEDOS[playerid], tstr);

				format(tstr, sizeof(tstr), "%f", nDamage);
				PlayerTextDrawSetString(playerid, HEALTH[playerid], tstr);
			}
			else
			{
...
ptask PlayerUpdate[999](playerid)
{
	...
	if(pData[playerid][pHBEMode] == 2 && pData[playerid][IsLoggedIn] == true)
	{
		...
		PlayerTextDrawTextSize(playerid,FiveM13[playerid], hunger, 23.0); //Seusain in aja size nya sama di textdraw nya
	 	PlayerTextDrawShow(playerid, FiveM13[playerid]);
	 	
	 	energy = pData[playerid][pEnergy] * 23.0/100;
		PlayerTextDrawTextSize(playerid,FiveM14[playerid], energy, 23.0); //Seusain in aja size nya sama di textdraw nya
	 	PlayerTextDrawShow(playerid,FiveM14[playerid]);
	...
	if(pData[playerid][pHospital] == 1)
    {
        ...
        if(pData[playerid][pHospitalTime] >= 15)
        {
			...
			SetPlayerPosition(playerid, 1835.39, -1338.41, 13.58, 261.40);
...
stock SendDiscordMessage(channel, message[]) {
	...
	switch(channel)
	{
		...
		case 1:
		{
			ChannelId = DCC_FindChannelById("985464734415458315");
			DCC_SendChannelMessage(ChannelId, message);
			return 1;
		}
		//==[ Log Chat IC ]
		case 2:
		{
			ChannelId = DCC_FindChannelById("985464504617934918");
			DCC_SendChannelMessage(ChannelId, message);
			return 1;
		}
		//==[ Warning & Banned ]
		...
		case 5:
		{
			ChannelId = DCC_FindChannelById("1205692301335339018");
			DCC_SendChannelMessage(ChannelId, message);
			return 1;
		}
		case 6://Korup
