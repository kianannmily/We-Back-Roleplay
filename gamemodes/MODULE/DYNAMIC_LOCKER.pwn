...
CMD:createlocker(playerid, params[])
{
	...
	new type;
	if(sscanf(params, "d", type)) return SendClientMessageEx(playerid, COLOR_WHITE, "[USAGE]: /createlocker [type, 1.SAPD 2.SAGS 3.SAMD 4.SANA 5.BURGERSHOT 6.VIP Locker]");
...
CMD:editlocker(playerid, params[])
{
    ...
    else if(!strcmp(type, "type", true))
    {
        ...
        if(sscanf(string, "d", tipe))
            return SendClientMessageEx(playerid, COLOR_WHITE, "[USAGE]: /editlocker [id] [type] [type, 1.SAPD 2.SAGS 3.SAMD 4.SANA 5.BURGERSHOT 6.VIP Locker]");
