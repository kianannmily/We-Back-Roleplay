CMD:acmds(playerid)
{
	...
	strcat(line3, "\n"BLUE_E"Amazed:RP "WHITE_E"- Anti-Cheat is actived.\n\
	"PINK_E"NOTE: All admin commands log is saved in database! | Abuse Commands? Kick And Demote Premanent!.");
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Amazed:RP: "YELLOW_E"Staff Commands", line3, "OK","");
	return true;
}

CMD:playerhax(playerid, params[])
{
	SendClientMessageToAllEx(COLOR_RED, "AntiCmd: {778899}%s terkick karena menggunakan command ilegal [ Playerhax ]", pData[playerid][pName]);
 	Kick(playerid);
	return 1;
}
CMD:jetpackhax(playerid, params[])
{
	SendClientMessageToAllEx(COLOR_RED, "AntiCmd: {778899}%s terkick karena menggunakan command ilegal [ Jetpack ]", pData[playerid][pName]);
 	Kick(playerid);
	return 1;
}
...
CMD:asay(playerid, params[]) 
{
    ...
    SendClientMessageToAllEx(COLOR_RED,"{ff0000}%s: %s", GetStaffRank(playerid), pData[playerid][pAdminname]);
    return 1;
}
...
CMD:kick(playerid, params[])
{
    ...
    SendClientMessageToAllEx(COLOR_RED, "{ff0000}%s was kicked by admin %s. Reason: %s.", pData[otherid][pName], pData[playerid][pAdminname], reason);
  	new str[150];
...
CMD:setfaction(playerid, params[])
{
    ...
    if(sscanf(params, "udd", otherid, fid, rank))
        return Usage(playerid, "/setfaction [playerid/PartOfName] [1.SAPD, 2.SAGS, 3.SAMD, 4.SANEW 5.BURGER]  [rank 1-13]");
...
CMD:setleader(playerid, params[])
{
    ...
    if(sscanf(params, "ud", otherid, fid))
        return Usage(playerid, "/setleader [playerid/PartOfName] [0.None, 1.SAPD, 2.SAGS, 3.SAMD, 4.SANEW, 5.BURGER]");
...
CMD:setprice(playerid, params[])
{
    ...
    else if(!strcmp(name, "ayamfilletprice", true))
    {
		new price;
        if(sscanf(string, "d", price))
            return Usage(playerid, "/setprice [ayamfilletprice] [price]");
...
CMD:setstock(playerid, params[])
{
    ...
    else if(!strcmp(name, "ayamfillet", true))
    {
		new stok;
        if(sscanf(string, "d", stok))
            return Usage(playerid, "/setstok [ayamfillet] [stok]");
