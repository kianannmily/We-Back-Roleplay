...
CMD:giveidcard(playerid, params[])
{
	...
	new otherid;
	if(sscanf(params, "u", otherid)) return Usage(playerid, "/giveidcard [playerid/PartOfName]");
...
{
	Error(playerid, "Removed Pm system");
}
...
CMD:kejar(playerid, params[])
{
	new targetid;
	if(sscanf(params, "i", targetid)) return Usage(playerid, "/kejar [Playerid]");
	if(!IsPlayerConnected(targetid)) return Error(playerid, "Player tersebut sedang tidak berada di kota!");

	pData[playerid][pCheckpointTarget] = SetTimerEx("CheckpointKejar", 100, true, "ii", playerid, targetid);
	return 1;
}

CMD:mkejar(playerid, params[])
{
...
function CheckpointKejar(playerid, targetid)
{
	if(!IsPlayerConnected(targetid))
	{
		KillTimer(pData[playerid][pCheckpointTarget]);
		Info(playerid, "Player tersebut tidak dapat dijangkau");
		DisablePlayerCheckpoint(playerid);
	}

	new Float:posx, Float:posy, Float:posz;
	GetPlayerPos(targetid, posx, posy, posz);
	SetPlayerCheckpoint(playerid, posx, posy, posz, 0.1);
	return 1;
}

CMD:bshop(playerid, params[])
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 291.35, -106.30, 1001.51)) return Error(playerid, "Anda sedang tidak berada di black market");
...
CMD:credits(playerid)
{
	...
	strcat(line3, ""LB_E"Coder: "YELLOW_E"Clairo for Update\n");
	strcat(line3, ""LB_E"Founder: "YELLOW_E"Ridho (Guluu)\n");
	strcat(line3, ""LB_E"Website: "YELLOW_E"https://discord.gg/Levitationrp\n");
	format(line2, sizeof(line2), ""LB_E"Server Support: "YELLOW_E"%s & All SA-MP Team\n\n\
	"GREEN_E"Terima kasih telah bergabung dengan kami! Copyright © 2022 | Levitation Roleplay.", pData[playerid][pName]);
	format(line1, sizeof(line1), "%s%s", line3, line2);
   	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Levitation:RP: "WHITE_E"Server Credits", line1, "OK", "");
	return 1;
}
...
CMD:vip(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Levitation:RP "PINK_E"VIP SYSTEM", longstr2, "Close", "");
	return 1;
}
...
CMD:time(playerid)
{
	...
	format(line2, sizeof(line2), ""WHITE_E"Paycheck Time: "YELLOW_E"%d remaining\n"WHITE_E"Delay Job: "RED_E"%d Detik\n"WHITE_E"Delay Side Job: "RED_E"%d Detik\n"WHITE_E"Plant Time(Farmer): "RED_E"%d Detik\n"WHITE_E"Arrest Time: "RED_E"%d Detik\n"WHITE_E"Jail Time: "RED_E"%d Detik\n", paycheck, pData[playerid][pJobTime], pData[playerid][pSideJobTime], pData[playerid][pPlantTime], pData[playerid][pArrestTime], pData[playerid][pJailTime]);
   	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Levitation RP: "WHITE_E"Time", line2, "Oke", "");
	return 1;
}
...
{
	if(pData[playerid][pBpjs] == 0) return Error(playerid, "Anda tidak memiliki bpjs card!");
	new sext[40];
	if(pData[playerid][pGender] == 1) { sext = "Male"; } else { sext = "Female"; }
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1330.39, 1566.83, 3010.90)) return Error(playerid, "Anda harus berada di rumah sakit!");
	if(pData[playerid][pBpjs] != 0) return Error(playerid, "Anda sudah memiliki Bpjs Card!");
	if(GetPlayerMoney(playerid) < 100) return Error(playerid, "Anda butuh $100 untuk membuat Bpjs Card");
	new sext[40], mstr[128];
	if(pData[playerid][pGender] == 1) { sext = "Laki-Laki"; } else { sext = "Perempuan"; }
	format(mstr, sizeof(mstr), "{FFFFFF}Nama: %s\nNegara: San Andreas\nTgl Lahir: %s\nJenis Kelamin: %s\nBerlaku hingga 14 hari!", pData[playerid][pName], pData[playerid][pAge], sext);
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "ID-Card", mstr, "Tutup", "");
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1672.12, -1347.85, 18.21)) return Error(playerid, "Anda harus berada di Bank!");
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1672.12, -1347.85, 18.21)) return Error(playerid, "Anda harus berada di bank point!");
	new tstr[128];
	...
	if(pData[playerid][pVip] > 0)
	{
		if(pData[playerid][pJob] == 0 || pData[playerid][pJob2] == 0)
		{
			if(pData[playerid][pJob] == 0)
			{
				...
				else if(pData[playerid][pJob] == 0 && GetPlayerState(playerid) == 1 && IsPlayerInRangeOfPoint(playerid, 3.0, 826.62, -613.77, 16.33))
				{
					pData[playerid][pGetJob] = 12;
					Info(playerid, "Anda telah berhasil mendaftarkan Job Pabrik Sparepart. /accept job untuk konfirmasi.");
				}
			...
			else if(pData[playerid][pJob2] == 0)
			{
				...
				else if(pData[playerid][pJob2] == 0 && GetPlayerState(playerid) == 1 && IsPlayerInRangeOfPoint(playerid, 3.0, 706.79, -666.39, 16.41))
				{
...
CMD:give(playerid, params[])
{
	...
	if(IsPlayerConnected(playerid)) 
	{
        ...
        if(sscanf(params, "us[24]d", otherid, name, ammount))
		{
			...
			Info(playerid, "Names: bandage, ayamfillet, cigarette, redmoney, potato, food, pizza, burger, chiken, cola, mineral, medicine, snack, sprunk, material, component, marijuana, obat, gps");
			return 1;
		}
