...
function OnPlayerDataLoaded(playerid, race_check)
{
	...
	{		
		...
		if(pData[playerid][pPassword] > 10)
		{
			...
			ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Login to Levitation", lstring, "Login", "Quit");
		}
...
function AssignPlayerData(playerid)
{
	...
	cache_get_value_name_int(0, "kecubung", pData[playerid][pKecubung]);
	cache_get_value_name_int(0, "paketborax", pData[playerid][pPaketborax]);
	cache_get_value_name_int(0, "paketkecubung", pData[playerid][pPaketkecubung]);
